...
def encrypt_button():
    ...
    if get_entries(password_entry) == value_list[0]: # şifre için koruma sağlama
    ...
    if not get_entries(title_entry) or not get_entries(note_text) or not get_entries(password_entry):
        ...
        value_list.clear()
    elif get_entries(password_entry) != value_list[0]:
...
def decrypt_button():
    try:
        secret = get_entries(note_text)
        if get_entries(password_entry) == value_list[0]:
...
button_maker("Save & Encrypt",0.5,0.89,lambda: [(get_password_value()),(encrypt_button()),(clear_button())])
